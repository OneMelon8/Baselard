...
public class GeneralTools {
	...
	public static String getBarFancy(double current, double max, double length) {
		// ▰▱
		final double percentage = Math.min(1.0, Math.round(current / max * 100D) / 100D);
		final int hashCount = (int) (percentage * length);
		final int dashCount = (int) (length - hashCount);
	...
	public static String signNumberEmotesPercentage(double decimal, boolean displayNumber) {
		if (decimal == 0)
			return Emotes.EQUALS + (displayNumber ? "0%" : "");
		if (decimal > 0)
			return Emotes.PLUS + (displayNumber ? getPercentage(decimal) : "");
		return Emotes.MINUS + (displayNumber ? getPercentage(Math.abs(decimal)) : "");
	}
